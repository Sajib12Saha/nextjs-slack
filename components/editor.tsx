...
const Editor = ({variant="create", onSubmit, onCancel, placeholder="Write something...", defaultValue=[], disabled=false, innerRef}:Props) => {
 ...
 return (
    <div className="flex flex-col">
        ...
        <div className={cn("flex flex-col border border-slate-200 rounded-md overflow-hidden focus-within:border-slate-300 focus-within:shadow-sm transition bg-white mb-3", disabled && "opacity-50")}>
       ...
       isEmpty?"bg-white hover:bg-white/80 text-muted-foreground" : "bg-[#007a5a] hover:bg-[#007a5a]/80 text-white")} size="iconSm" disabled={disabled || isEmpty} onClick={()=> {
        onSubmit({
            body:JSON.stringify(quillRef.current?.getContents()),
            image,
        })

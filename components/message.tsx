...
import { useToggleCreateReaction } from '@/app/(features)/reactions/api/use-toggle-create-reaction';
...
const formatFullTime = (date: Date) => {
    ...
    return `${isToday(date) ? "Today" : isYesterday(date) ? "Yesterday" : format(date, "MMMM d, yyyy")} at ${format(date, "h:mm a")}`
}
...
export const Message = ({
    ...
    const {mutate:toggleReaction, isPending:isTogglingReaction} = useToggleCreateReaction();

    const [ConfirmDialog, confirm] = useConfirm("Delete message", "Are you sure you want delete this message? This cannot be undone") 
                ...
                <button className="text-xs text-muted-foreground opacity-0 group-hover:opacity-100 w-[40px] leading-[22px] text-center hover:underline">
                 ...
                 {isEditing ? (
                    <div className="w-full h-full">
                        <Editor
                        onSubmit={handleUpdate} disabled={isPending} defaultValue={JSON.parse(body)} onCancel={()=> setEditingId(null)} variant="update"
                        />
                    </div>
                 ): (
        ...
        <div className={cn('flex flex-col gap-2 p-1.5 px-5 hover:bg-gray-100/60 group relative',
            ...
            <div className="flex items-start gap-2">
                <button onClick={()=> onOpenProfile(memberId)}>
                    <Avatar>
                    <AvatarImage alt="Author" src={authorImage}/>
